    ...
    public function getProducts() {
        ...
        foreach ($quote->getAllVisibleItems() as $item) {
            ...
            $productDetail['id'] = $this->helper->getGtmProductId($product);
             // Check if the item price is not null before formatting it
             $price = $item->getPriceInclTax() !== null ? (float)$item->getPriceInclTax() : 0.0;
             $productDetail['price'] = number_format($price, 2, '.', '');
