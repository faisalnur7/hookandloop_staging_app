...
namespace Ravedigital\Custom\Model\Carrier;
        ...
        foreach ($carrierRates as $carrierRate) {
            ...
            if (!array_key_exists('rates', $carrierRate)) {
                ...
                foreach ($carrierRate['rates'] as $rateDetails) {
                    ...
                    //Custom code to seprate delivery method title and date
                    if($rateDetails['carriergroup_detail']['methodTitle']){
                        $rate->setShippingTitle(__($rateDetails['carriergroup_detail']['methodTitle']));
                    }
                    if (array_key_exists('method_description', $rateDetails)) {
                        $rate->setEtaDeliveryDate(__($rateDetails['method_description']));
                    }
                    //
