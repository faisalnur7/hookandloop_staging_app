...
class Result extends \Magento\Framework\Model\AbstractModel implements IdentityInterface, OptionSourceInterface
{
    ...
    public function sendEmail($recipient = 'admin', $contact = false)
    {
        ...
        if ($recipient == 'admin') {
            $bcc_list = explode(',', (string)$webform->getBccAdminEmail());
        }

        if ($recipient == 'customer') {
            $bcc_list = explode(',', (string)$webform->getBccCustomerEmail());
        }
        // trim bcc array
        $bcc_list = array_map('trim', $bcc_list);

        $validateEmail = new \Laminas\Validator\EmailAddress();
    ...
    public function sendApprovalEmail()
    {
        ...
        if (!is_null($email)) {
            if(is_array($email) && count($email) == 1) $email = $email[0];
            @$this->_transportBuilder
                ->setTemplateIdentifier($templateId)
                ->setTemplateOptions(
                    [
            ...
            $bcc_list = explode(',', (string)$webform->getBccApprovalEmail());
            // trim bcc array
    ...
    public function getEmailSubject($recipient = 'admin')
    {
        ...
        if ($recipient == 'customer') {
            $subject = __("You have submitted '%1' form on %2 website", $webform_name, $store_name)->render();
        }

        //iterate through fields and build subject
        $subject_array       = [];
        $fields_to_fieldsets = $webform->getFieldsToFieldsets(true);
        $fields_to_fieldsets['hidden'] = ['fields' => $webform->_getHidden()];
    ...
    {
        return long2ip((int)$this->getCustomerIp());
    }
