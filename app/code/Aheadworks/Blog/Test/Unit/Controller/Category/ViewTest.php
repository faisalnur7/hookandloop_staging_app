...
class ViewTest extends \PHPUnit\Framework\TestCase
{
    ...
    const CATEGORY_META_KEYWORDS = 'Meta keywords';
    const STORE_ID = 1;
    ...
    private function prepareCategoryMock($status = self::CATEGORY_STATUS, $storeId = null)
    {
        ...
        $this->categoryMock->expects($this->any())
            ...
            ->will($this->returnValue(self::CATEGORY_META_KEYWORDS));
        $this->categoryMock->expects($this->any())
            ->method('getStatus')
    ...
    public function testExecutePageConfig()
    {
        ...
        $this->pageConfigMock->expects($this->any())
            ->method('setMetadata')
            ->withConsecutive(
                [$this->equalTo('keywords'), $this->equalTo(self::CATEGORY_META_KEYWORDS)],
                [$this->equalTo('description'), $this->equalTo(self::CATEGORY_META_DESCRIPTION)]
            );
        $this->action->execute();
    }
