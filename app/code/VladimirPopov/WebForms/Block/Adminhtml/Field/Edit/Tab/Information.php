...
class Information extends \Magento\Backend\Block\Widget\Form\Generic implements \Magento\Backend\Block\Widget\Tab\TabInterface
{
    ...
    protected function _prepareForm()
    {
        ...
        $password_encrypt = $fieldset->addField('value_password_encrypt', 'select', [
            'label' => __('Encrypt password'),
            'required' => false,
            'name' => 'value[password_encrypt]',
            'note' => 'It is recommended to store passwords encrypted for better security',
            'values' => ['1' => __('Yes'), '0' => __('No')]
        ]);
        ...
        $this->setChild('form_after', $this->getLayout()->createBlock('Magento\Backend\Block\Widget\Form\Element\Dependence', 'fields_information_dependence')
            ...
            ->addFieldMap($password_encrypt->getHtmlId(), $password_encrypt->getName())
            ->addFieldDependence(
                $hint->getName(),
                $type->getName(),
                'text'
