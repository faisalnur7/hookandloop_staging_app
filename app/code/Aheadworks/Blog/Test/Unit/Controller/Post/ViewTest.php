...
class ViewTest extends \PHPUnit\Framework\TestCase
{
    ...
    const POST_META_KEYWORDS = 'Meta keywords';
    const CATEGORY_ID = 1;
    const STORE_ID = 1;
    ...
    /**
     * @var TitleResolver|\PHPUnit_Framework_MockObject_MockObject
     */
    private $titleResolverMock;
    ...
    public function setUp() : void
    {
        ...
        $this->resultPageMock = $this->getMockBuilder(Page::class)
            ->setMethods(['getConfig', 'addPageLayoutHandles'])
            ->disableOriginalConstructor()
            ->getMock();
        ...
        $this->titleResolverMock = $this->createMock(TitleResolver::class);
        ...
        $configMock = $this->getMockBuilder(BlogConfig::class)
            ->setMethods(['getSeoUrlType', 'getPostCanonicalTag', 'areMetaTagsEnabled'])
            ->disableOriginalConstructor()
            ->getMock();
        ...
        $this->action = $objectManager->getObject(
            ...
            [
                ...
                'titleResolver' => $this->titleResolverMock
            ]
        );
    }
    ...
    private function preparePostMock($status = self::POST_STATUS, $storeId = null, $categoryIds = null)
    {
        ...
        $this->titleResolverMock->expects($this->any())
            ->method('getTitle')
            ->will($this->returnValue(self::POST_META_TITLE));
        $this->postMock->expects($this->any())
            ->method('getMetaKeywords')
            ->will($this->returnValue(self::POST_META_KEYWORDS));
        $this->postMock->expects($this->any())
    ...
    public function testExecutePageConfig()
    {
        ...
        $this->pageConfigMock->expects($this->any())
            ->method('setMetadata')
            ->withConsecutive(
                [$this->equalTo('keywords'), $this->equalTo(self::POST_META_KEYWORDS)],
                [$this->equalTo('description'), $this->equalTo(self::POST_META_DESCRIPTION)]
            );
        $this->action->execute();
    }
