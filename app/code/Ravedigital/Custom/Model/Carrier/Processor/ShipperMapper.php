...
namespace Ravedigital\Custom\Model\Carrier\Processor;

use ShipperHQ\Shipper\Model\Carrier\Processor\ShipperMapper as OriginalShipperMapper;
use ShipperHQ\Shipper\Model\Carrier\Processor\StockHandlerFactory;
...
class ShipperMapper extends OriginalShipperMapper
{
    ...
    {
        // die("shub");
         $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/shipperhq.log');
         $logger = new \Zend\Log\Logger();
         $logger->addWriter($writer);
        
        $logger->info('test shipperHQ'); 
        $formattedItems = [];
        ...
        foreach ($magentoItems as $magentoItem) {
            ...
            $storePrice = $this->getItemStorePrice($magentoItem)*$measurement_sold_in_size;
            ...
            $formattedItem = $this->itemFactory->create([
                ...
                'basePrice'                   => $magentoItem->getBasePrice()*$measurement_sold_in_size,
                'baseRowTotal'                => $magentoItem->getBaseRowTotal(),
                'discountAmount'              => (float)$discountAmount,
                'discountPercent'             => (float)$magentoItem->getDiscountPercent(),
                'discountedBasePrice'         => $magentoItem->getBasePrice()*$measurement_sold_in_size -
                                                    ($baseDiscountAmount / $magentoItem->getQty()),
                'discountedStorePrice'        => $storePrice - ($discountAmount / $magentoItem->getQty()),
                'discountedTaxInclBasePrice'  => $magentoItem->getBasePrice()*$measurement_sold_in_size -
                                                    ($baseDiscountAmount / $magentoItem->getQty()) +
