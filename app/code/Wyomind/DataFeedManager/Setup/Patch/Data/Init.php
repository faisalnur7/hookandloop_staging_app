...
class Init implements DataPatchInterface, PatchRevertableInterface
{
    ...
    public function __construct(
        ...
        AttributeCollectionFactory $entityAttributeCollectionFactory
    ) {
    ...
    public function apply()
    {
        ...
        if (version_compare($this->dataVersion, self::version) >= 0) {
            ...
            $data = ["templates" => [], "functions" => [], "variables" => []];
            $data['templates'][] = [
                "id" => null,
                "name" => "GoogleShopping",
                "type" => 1,
...
<feed>',
            ...
            // plytix
            $plytixColumns = ["SKU" => "{{product.sku}}",
                              "Variation of" => "{{parent.sku}}",
                              "Label" => "{{product.name}}",
                              "Categories" => "{{product.categories nth=-1 | parent.categories nth=-1}}",
                              "Files" => "{{parent.url | product.url}}",
                              "Color" => "{{product.color}}",
                              "Description" => "{{product.description | parent.description}}",
                              "Price" => "{{product.price if=\\\"product.type_id!=configurable\\\" | product.min_price if=\\\"product.type_id==configurable\\\"}}"
            ];

            $attributesList = $this->entityAttributeCollectionFactory->create()->setEntityTypeFilter(4);
