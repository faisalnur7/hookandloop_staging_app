 ...
 */
namespace Exinent\Checkout\Controller\Onepage;

class SaveOrder extends \Magento\Checkout\Controller\Onepage\SaveOrder
{
    /**
     * Create order action
     *
    ...
    public function execute()
    {
     ...
     if($isCheckoutLoggerEnabled) {
        ...
        $logger->info('Checkout app/code/Exinent/Checkout/Controller/Onepage/SaveOrder: ');
    }
    ...
    try {
        ...
        if (!$agreementsValidator->isValid(array_keys($this->getRequest()->getPost('agreement', [])))) {
            ...
            if($isCheckoutLoggerEnabled) {
                $logger->info('Checkout app/code/Exinent/Checkout/Controller/Onepage/SaveOrder:- Please agree to all the terms and conditions before placing the order.');
            }

            return $this->resultJsonFactory->create()->setData($result->getData());
        }
        $shippingMethod=$this->getRequest()->getPost('shipping_method');
        $customShippingDetails = $this->getRequest()->getPost('exinent_shippingoption', array());
        ...
        if($dupOrder) {
           if($isCheckoutLoggerEnabled) {
            $logger->info('Checkout app/code/Exinent/Checkout/Controller/Onepage/SaveOrder:- Duplicate Order.');
        }
    ...
    if($isCheckoutLoggerEnabled) {
        $logger->info('Checkout app/code/Exinent/Checkout/Controller/Onepage/SaveOrder:- Catch1 -'. $message);
    }
    ...
    if($isCheckoutLoggerEnabled) {
        $logger->info('Checkout app/code/Exinent/Checkout/Controller/Onepage/SaveOrder:- Catch2 -'.$e->getMessage());
    }
    ...
    if($isCheckoutLoggerEnabled) {
        $logger->info('Checkout app/code/Exinent/Checkout/Controller/Onepage/SaveOrder:- Catch3 -'.$e->getMessage());
    }
}
        /**
