...
class Field extends AbstractModel implements IdentityInterface
{
    ...
    public function getComment()
    {
        ...
        $text    = trim(preg_replace($this->tooltip_clean_regex, "", (string)$comment));
        $filter  = $this->_filterProvider->getPageFilter();
        if ($text)
            return $this->applyTranslation($filter->filter($text));
        return '';
    }

    /**
     * Get CSV translation if its enabled
     * @param $str
     * @return Phrase
     */
    public function applyTranslation($str)
    {
    ...
    public function getTooltip($option = false)
    {
        ...
        if ($option) {
            ...
            preg_match_all($pattern, (string)$comment, $matches);
            if (!empty($matches[1]))
                foreach ($matches[1] as $i => $match) {
                    if (trim((string)$match) == trim((string)$option)) {
                    }
                        return $this->applyTranslation($matches[2][$i]);
                }
            return false;
        }

        preg_match($pattern, (string)$comment, $matches);

        if (!empty($matches[1])) {
            return $this->applyTranslation(trim((string)$matches[1]));
        }
    ...
    {
        $value = preg_replace($this->img_regex, "", (string)$value);
        $value = str_replace('{{null}}', '', (string)$value);
        $value = str_replace('{{disabled}}', '', (string)$value);

        if ($this->isNullOption($value) && substr((string)$value, 0, 2) == '^^')
            return trim(substr((string)$value, 2));
        if (substr((string)$value, 0, 1) == '^')
            return trim(substr((string)$value, 1));
    ...
    public function getHiddenFieldValue()
    {
        ...
        if ($field_value) {
            return trim(filter_var($filter->filter($field_value), FILTER_SANITIZE_FULL_SPECIAL_CHARS));
        }
        return '';
    }
    ...
    public function toHtml()
    {
        ...
        if(is_array($field_value)){
            $field_value = array_map(function($value) { return str_replace('{{var', '\{\{var', (string)$value); }, $field_value);
        }
        if(is_string($field_value)){
            $field_value = str_replace('{{var','\{\{var',$field_value);
        }
        ...
        //apply page filter
        $field_value = array_map([$this, 'applyPageFilter'], $field_value);
        if (is_array($field_value))
        ...
        switch ($field_type) {
            case 'text':
                ...
                break;
            case 'url':
                $config["placeholder"] = $this->getHintUrl();
                $config["field_class"] .= " input-text";
                if (!$customer_value) empty($field_value['text']) ? $config['field_value'] = '' : $config['field_value'] = $field_value['text'];
                break;
            case 'email':
                $config['placeholder'] = $this->getHintEmail();
                $config["field_class"] .= " input-text";
            ...
            case 'autocomplete':
                ...
                break;
            case 'textarea':
                $config['placeholder'] = $this->getHintTextarea();
            ...
            case 'html':
                ...
                break;
            case 'datetime':
                $config['placeholder'] = $this->getHintDatetime();
                ...
                break;
            case 'date':
                $config['placeholder'] = $this->getHintDate();
                $config["field_class"] .= " input-text";
                $calendar              = true;
                $config['accdc']       = $this->_scopeConfig->getValue('webforms/accessibility/accdc_calendar', ScopeInterface::SCOPE_STORE);
                if ($customer_value) {
        ...
        if ($this->getData('validate_regex')) {
            ...
            $validate_message = trim(str_replace("'", "\'", (string)$this->getData('validate_message')));
...
<script>
    ...
    {
        return $this->applyTranslation($this->getData('hint'));
    }

    private function getHintUrl()
    {
        return $this->applyTranslation($this->getData('hint_url') ?: $this->getHint());
    }

    private function getHintEmail()
    {
        return $this->applyTranslation($this->getData('hint_email') ?: $this->getHint());
    }

    private function getHintTextarea()
    {
        return $this->applyTranslation($this->getData('hint_textarea') ?: $this->getHint());
    }
    ...
    private function getHintDatetime()
    {
        return $this->getData('hint_datetime') ?: $this->getHint();
    }

    private function getHintDate()
    {
        return $this->getData('hint_date') ?: $this->getHint();
    }
