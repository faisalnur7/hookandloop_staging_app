...
class Collection extends \Magento\Catalog\Model\ResourceModel\Product\Option\Collection
{
    ...
    public function getCustomOptions()
    {
        ...
        $this->getSelect()
            ->columns(['product_id'])
            ->joinleft(["cpot" => $tableCpot], "cpot.option_id=main_table.option_id AND cpot.store_id=0", ["option" => "title", "option_id", "store_id", "option_title_id"])
            ->joinleft(["cpotv" => $tableCpotv], "cpotv.option_id = main_table.option_id", "sku")
            ->joinleft(["cpott" => $tableCpott], "cpott.option_type_id=cpotv.option_type_id AND cpott.store_id=cpot.store_id", ["title AS value","option_type_id"])
        ...
        foreach ($r as $customOption) {
            $customOptions[$customOption["product_id"]][$customOption["option"]]["options"][] = [
                ...
                "value_id" => $customOption["option_type_id"],
