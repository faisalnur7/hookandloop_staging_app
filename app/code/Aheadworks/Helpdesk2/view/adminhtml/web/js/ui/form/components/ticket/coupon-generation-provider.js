...
], function (_, registry, Provider, messageManager, showAlertMessage, reloadComponent, updateDataSource, quickResponseContent) {
    'use strict';
        ...
        onResponse: function (response) {
            if (_.isObject(response)) {
                if (response.error) {
                    ...
                    var content = "<br>" + response.data.couponMessage + "<br>" + response.data.discountMessage
                    quickResponseContent.setContent(content)
                }
            }
        }
    });
