    ...
    public function getList(\Magento\Framework\Api\SearchCriteriaInterface $searchCriteria)
    {
        ...
        foreach ($searchCriteria->getFilterGroups() as $filterGroup) {
            ...
            foreach ($filterGroup->getFilters() as $filter) {
                ...
                } elseif ($filter->getField() == 'tag_name') {
                    $collection->addTagNameFilter($filter->getValue());
                }elseif ($filter->getField() == 'category_name') {
                    $collection->addCategoryNameFilter($filter->getValue());
                } elseif ($filter->getField() == 'category_url_key') {
                    $collection->addCategoryUrlKeyFilter($filter->getValue());
                } elseif ($filter->getField() == 'author_url_key') {
                    $collection->addAuthorUrlKeyFilter($filter->getValue());
